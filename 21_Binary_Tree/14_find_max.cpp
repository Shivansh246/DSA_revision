...
int findMax(BTnode<int> *root){
    ...
    return max(ans,max(rightmax,leftmax));
}
void findMaxOtherWay(BTnode<int> *root,int &ans){
    if(root==NULL){
        return ;
    }
    ans  = max(ans,root->data);
    findMaxOtherWay(root->left,ans);
    findMaxOtherWay(root->right,ans);
}
int main(){
    ...
    cout<<endl<<"maximum value: "<<findMax(root);
    delete root;
    return 0;
}