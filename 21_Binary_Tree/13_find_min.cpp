...
int findMin(BTnode<int> *root){
    ...
    return min(ans,min(rightmin,leftmin));
}
void findMInOtherWay(BTnode<int> *root,int &ans){
    if(root==NULL){
        return ;
    }
    ans  = min(ans,root->data);
    findMInOtherWay(root->left,ans);
    findMInOtherWay(root->right,ans);
}
int main(){
    BTnode<int> *root =takeInput();
    printTree(root);
    int ans = INT_MAX;
    findMInOtherWay(root,ans);
    cout<<endl<<"minimum value: "<< ans;
    delete root;
    return 0;
}